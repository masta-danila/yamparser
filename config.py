...
RECHECK_DAYS = 62                       # Дней для повторной проверки отзывов «Размещен» (если нет на карточке -> «Удален»)
RECHECK_DAYS_DELTA = 30                 # Запас (дней) на ошибки в дате — парсим глубже, чтобы не пометить «Удален» по ошибке
